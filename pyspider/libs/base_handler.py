...
from pyspider.libs.utils import md5string, timeout, pickle_dumps, pickle_loads
...
class BaseHandler(object):
    ...
    fetch_fields = ('method', 'headers', 'user_agent', 'data', 'connect_timeout', 'timeout', 'allow_redirects', 'cookies',
                    ...
                    'max_redirects', 'robots_txt', 'validate_cert', 'min_wait_time')
    ...
    def crawl_handler_page(self, handler_page):
        ...
        crawl_builder \
            .set_save_kv('handler_page', pickle_dumps(handler_page)) \
            .set_save_kv('page_name', sys.modules[handler_page.__module__].__name__) \
            .set_kwargs_kv('callback', self.handler_page_callback)
        return self.crawl(crawl_builder.get_url(), **crawl_builder.get_kwargs())

    def handler_page_callback(self, response, task):
        """
        自定义封装的handler爬虫回调方法
        :param response:
        :param task:
        :return:
        """
        handler_page = pickle_loads(response.save['handler_page'])
        result = handler_page.parse_response(response, task)
        for _ in handler_page.get_follows():
            self.crawl_handler_page(_)
        for _ in handler_page.get_messages():
            msg = _.get('msg', {})
            url = _.get('url', 'data:,on_message')
