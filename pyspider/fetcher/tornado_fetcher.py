...
from pyspider.helper.logging import task_log, fetcher_logger
from pyspider.libs import utils, dataurl, counter
from pyspider.libs.url import quote_chinese
from pyspider.libs.utils import pickle_loads
...
class Fetcher(object):
    ...
    def async_fetch(self, task, callback=None):
        ...
        if isinstance(save, dict) and 'handler_page' in save:
            ...
            crawl_builder = handler_page.crawl_builder()
            proxy = crawl_builder.get_proxy()
            if proxy:
                task['fetch']['proxy'] = proxy
            cookies_str = crawl_builder.get_cookies_str()
            if cookies_str:
                task['fetch']['headers']['Cookie'] = cookies_str
            cookies_dict = crawl_builder.get_cookies_dict()
            if cookies_dict:
                task['fetch']['cookies'] = cookies_dict
    ...
    def http_fetch(self, url, task):
        '''HTTP fetcher'''
        task_log(task, 'http fetch')
        time_list = []
        start_time = time.time()
        time_list.append({'time': start_time, 'step': 'start'})
        self.on_fetch('http', task)
        handle_error = lambda x: self.handle_error('http', url, task, start_time, x)

        # setup request parameters
        fetch = self.pack_tornado_request_parameters(url, task)
        time_list.append({'time': time.time(), 'step': 'setup request parameters'})
        task_fetch = task.get('fetch', {})
        ...
        if 'cookies' in fetch:
            ...
            del fetch['cookies']
        time_list.append({'time': time.time(), 'step': 'set session'})
        ...
        while True:
            ...
            time_list.append({'time': time.time(), 'step': 'before request'})
            ...
            try:
                ...
                if e.response:
                    ...
                    raise gen.Return(handle_error(e))
            time_list.append({'time': time.time(), 'step': 'after request'})
            ...
            result['headers']['time_list'] = time_list
    ...
    def on_result(self, type, task, result):
        ...
        task_log(task, 'fetch result', extra={'status_code': status_code, 'time': result.get('time')})
