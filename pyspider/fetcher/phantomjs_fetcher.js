...
if (system.args.length !== 2) {
  ...
  service = server.listen(port, {
    ...
    min_wait_time = fetch.min_wait_time ? fetch.min_wait_time * 1000 : 0;
    if (fetch.proxy) {
    ...
    page.onLoadFinished = function(status) {
      ...
      console.debug("waiting "+Math.max(wait_before_end,min_wait_time)+"ms before finished.");
      end_time = Date.now() + Math.max(wait_before_end,min_wait_time);
      setTimeout(make_result, Math.max(wait_before_end,min_wait_time)+10, page);
    };
