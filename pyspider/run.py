...
def scheduler(ctx, xmlrpc, xmlrpc_host, xmlrpc_port,
    ...
    """
    config.set_process_name(config.CONST_PROCESS_SCHEDULER)
    from pyspider.helper.logging import scheduler_logger
    scheduler_logger.info('start')
    g = ctx.obj
...
def fetcher(ctx, xmlrpc, xmlrpc_host, xmlrpc_port, poolsize, proxy, user_agent,
    ...
    Run Fetcher.
    """
    config.set_process_name(config.CONST_PROCESS_FETCHER)
    from pyspider.helper.logging import fetcher_logger
    fetcher_logger.info('start')
    g = ctx.obj
...
def processor(ctx, processor_cls, process_time_limit, enable_stdout_capture=True, get_object=False):
    ...
    Run Processor.
    """
    config.set_process_name(config.CONST_PROCESS_PROCESSOR)
    from pyspider.helper.logging import processor_logger
    processor_logger.info('start')
    g = ctx.obj
...
def result_worker(ctx, result_cls, get_object=False):
    ...
    Run result worker.
    """
    config.set_process_name(config.CONST_PROCESS_RESULT)
    from pyspider.helper.logging import result_logger
    result_logger.info('start')
    result_logger.info('load_cls: %s' % result_cls)
    g = ctx.obj
...
def webui(ctx, host, port, cdn, scheduler_rpc, fetcher_rpc, max_rate, max_burst,
    ...
    Run WebUI
    """
    config.set_process_name(config.CONST_PROCESS_WEBUI)
    from pyspider.helper.logging import webui_logger
    webui_logger.info('start')
    ...
    app.run(host=host, port=port, debug=config.getboolean('webui', 'debug'))
...
def phantomjs(ctx, phantomjs_path, port, auto_restart, args):
    ...
    """
    config.set_process_name(config.CONST_PROCESS_WEBUI)
    from pyspider.helper.logging import phantomjs_logger
    phantomjs_logger.info('start')
    ...
    try:
        ...
        phantomjs_logger.warning('phantomjs not found, continue running without it.')
        return None
    ...
    def quit(*args, **kwargs):
        ...
        _phantomjs.kill()
        _phantomjs.wait()
        phantomjs_logger.info('phantomjs exited.')
