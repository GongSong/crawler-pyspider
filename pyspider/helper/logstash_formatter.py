'''
from https://github.com/ulule/python-logstash-formatter
...
class LogstashFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        logr.update({'@message': msg,
                     ...
                     'datetime': datetime.datetime.now().strftime('%Y-%m-%dT%H:%M:%S+08:00'),
...
class LogstashFormatterV1(LogstashFormatter):
    ...
    def format(self, record):
        ...
        if 'args' in fields:
            task = fields.pop('args')
            if 'taskid' in task:
                fields.setdefault('task', {}).update({
        ...
        base_log = {'@timestamp': utcnow.strftime("%Y-%m-%dT%H:%M:%S") +
                                  ".%03d" % (utcnow.microsecond / 1000) + "Z",
                    'datetime': tznow.strftime('%Y-%m-%dT%H:%M:%S+08:00'),
                    '@version': 1,
                    'source_host': self.source_host}
