...
class ModifyAppointmentOutbound(Base):
    """
    修改 预约出库单
    """
    request_data = """
    <batch>
    <request type="json"><![CDATA[{"action":"resolve-data","dataResolver":"inventoryInOutboundInterceptor#saveAppointmentBill","dataItems":[{"alias":"dsBill","data":{"$isWrapper":true,"$dataType":"v:inventory.outboundAppointment$[appointmentBill]","data":{
    %s,
    "details":{"$isWrapper":true,"$dataType":"v:inventory.outboundAppointment$[appointmentDetail]",
    "data":%s
    ...
    def __init__(self, post_data):
        super(ModifyAppointmentOutbound, self).__init__()
    ...
    def get_unique_define(self):
        return merge_str('modify_appointment_outbound', self.__remark, self.__storage_uid)
...
if __name__ == '__main__':
    ...
    ModifyAppointmentOutbound(post_data).use_cookie_pool().get_result()
