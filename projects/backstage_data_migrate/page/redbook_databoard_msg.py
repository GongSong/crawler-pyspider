...
class RedBookDataBoardMsg(BaseCrawl):
    """
    小红书后台数据获取---流量看板
    """
    URL = 'https://ark.xiaohongshu.com/api/ark/chaos/trd/realtime/overall?current_date={}'
    website = 'redbook'

    def __init__(self, date, username, retry=RETRY_TIMES):
        super(RedBookDataBoardMsg, self).__init__()
        self.__date = date
        self.__url = self.URL.format(self.__date)
        self.__retry = retry
    ...
    def parse_response(self, response, task):
        ...
        if code is not True:
            if self.__retry > 0:
                ...
                self.__retry -= 1
                self.crawl_handler_page(RedBookDataBoardMsg(self.__date, self.__retry, self.__username))
            else:
