...
class JDTrade(BaseCrawl):
    """
    获取京东交易概况信息
    """
    URL = 'https://sz.jd.com/sz/api/trade/getSummaryData.ajax?channel=99&cmpType=0&date={date}&endDate={end_date}' \
          '&startDate={start_date}#{id}'
    website = 'jingdong'
    random_id = uuid.uuid4()

    def __init__(self, date, end_date, start_date, date_type_name, username, retry=RETRY_TIMES):
        super(JDTrade, self).__init__()
        self.__date = date
        ...
        if int(summary_code) != 0:
            ...
            if self.__retry < 1:
                ...
                self.crawl_handler_page(
                    JDTrade(self.__date, self.__end_date, self.__start_date, self.__date_type_name, self.__username,
                            self.__retry))
        else:
