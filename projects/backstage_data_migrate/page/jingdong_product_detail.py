...
class JDProdictDetail(BaseCrawl):
    """
    获取京东商品概况数据
    """
    URL = 'https://sz.jd.com/sz/api/productDetail/getProductSummary.ajax?channel=99&cmpType=0&date={date}' \
          '&endDate={end_date}&startDate={start_date}&type=0#{id}'
    website = 'jingdong'
    random_id = uuid.uuid4()

    def __init__(self, date, end_date, start_date, date_type_name, username, retry=RETRY_TIMES):
        super(JDProdictDetail, self).__init__()
        self.__date = date
        ...
        if int(summary_code) != 0:
            ...
            if self.__retry < 1:
                ...
                self.crawl_handler_page(
                    JDProdictDetail(self.__date, self.__end_date, self.__start_date, self.__date_type_name,
                                    self.__username, self.__retry))
        else:
