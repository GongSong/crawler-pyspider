...
from blog_spider.page.weibo_article_result import WeiboArticleRlt
...
class WeiboBloggerRlt(BaseCrawl):
    ...
    def parse_response(self, response, task):
        ...
        if cards:
            ...
            for _card in cards:
                ...
                # 用户的头像
                avatar = dict()
                avatar_img = mblog['user']['profile_image_url']
                ...
                self.catch_image(avatar_img, avatar_img_path, self.__priority)
                ...
                if img_data:
                    for _img in img_data:
                        ...
                        self.catch_image(img_url, img_path, priority=self.__priority)
                ...
                if not blue_vip:
                    WeiboArticleRlt(full_content_url, article_data, use_proxy=True,
                                    priority=self.__priority).get_result()
            else:
                # self.send_inform()
                pass
        else:
    ...
    def catch_image(self, avatar_img, avatar_img_path, priority, retry=3):
        try:
            BlogImgDown(avatar_img, avatar_img_path, priority=priority).get_result()
        except Exception as e:
            print('catch_image error : {}'.format(e))
            if retry > 0:
                self.catch_image(avatar_img, avatar_img_path, priority, retry - 1)
