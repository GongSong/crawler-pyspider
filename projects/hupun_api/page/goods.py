...
class Goods(Base):
    PATH = '/erp/item/goods/query'
    """
    商品信息 的数据抓取
    万里牛的商品API的商品添加操作不能传入颜色和尺码参数，不能用来同步商品
    """

    def __init__(self, to_next_page=False, post_data=None):
        super(Goods, self).__init__(self.PATH, post_data)
    ...
    def crawl_next_page(self):
        ...
        self.crawl_handler_page(Goods(to_next_page=self.__to_next_page, post_data=post_data))
        pass
        # if self._post_data:
