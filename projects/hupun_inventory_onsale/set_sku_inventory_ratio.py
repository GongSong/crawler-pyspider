...
class SetSkuSearchRatio(Base):
    '''设置sku级别库存例外宝贝比例'''
    # storeuid_dict = StoreHouse().get_all_name_uid_dict()
    max_threads = 6

    def execute(self):
        print('开始设置sku的库存例外宝贝')
        self.print_basic_info()
        data = self._data

        total_amount = data.get('total')
        missions_amount = data.get('taskNum')
        mission = 'sku_set_ratio_mission'
        status = 'sku_set_ratio_status'
        progress = 'sku_set_ratio_progress'
        total = 'sku_set_ratio_total'
    ...
    @progress_counter('sku_set_ratio_progress')
    def consume_task(self, _item_copy, queue):
        ...
        try:
            ...
            if result_first_para is True:
                _item_copy['failReason'] = ''
                _item_copy['syncStatus'] = '同步成功'
                _item_copy['syncTime'] = str(Date.now().millisecond())
                print('同步成功', _item_copy)
            if result_first_para is not True:
                _item_copy['failReason'] = result[1]
                _item_copy['syncStatus'] = '同步失败'
                _item_copy['syncTime'] = str(Date.now().millisecond())
                print('同步失败', _item_copy)
                error_msg = result[1]
    ...
    def consume(self, item):
        try:
            ...
            if '唯品会' in item['channel']:
                ...
                for _search_item in search_spu_list:
                    if _search_item['upload_status'] == 0:
                        error_msg = 'sku内有开关状态为双关的商品，无法成功设置库存比例'
                        return 'other', error_msg

                for search_item in search_spu_list:
                    VipSyncRatio(search_item['item_id'], search_item['upload_status'], shop_uid, quantity_type,
                                 upload_ratio,
                                 storage_uid, virtual_inventory).use_cookie_pool().get_result()
                return True

            if '小红书' in item['channel']:
                ...
                for _search_item in search_list_filter:
                    if _search_item['upload_status'] == 0:
                        error_msg = 'sku内有开关状态为双关的商品，无法成功设置库存比例'
                        return 'other', error_msg
            ...
            for search_spu in search_spu_list:
                ...
                if search_list == []:
                    ...
                    for search_item in search_list_filter:
                        print(search_item['outer_id'], search_item['item_id'], search_spu['upload_status'], shop_uid,
                              quantity_type, upload_ratio,
                              storage_uid, virtual_inventory, search_item['sku_id'])
                        InventorySyncUpload(search_item['item_id'], search_spu['upload_status'], shop_uid,
                                            quantity_type,
                                            upload_ratio,
                                            storage_uid, virtual_inventory, search_item['sku_id']).use_cookie_pool().get_result()
            return True
    ...
    def ding_talk_alarm(es_data, error_msg):
        ...
        title = '万里牛设置sku例外宝贝操作报警'
        content = es_data['channel'] + '的商品：' +  es_data['skuBarcode'] + '例外宝贝sku库存比例操作失败，原因：' + error_msg
